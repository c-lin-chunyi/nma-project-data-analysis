...
**r2 amendment after technical failure:** GitHub run `29646265282` produced no
complete hazard estimate or interpretable neural \(\Delta LL\). Nine HMM jobs
terminated with native XLA memory-allocation failures and the remaining mouse
was rejected by an implementation threshold inconsistent with this document.
Before any successful DEV analysis, r2 therefore fixes \(K=2\), moves \(K=1,3\)
to registered sensitivities, and introduces immutable fixed-shape HMM
checkpoints. The failed r1 draft and its provenance remain preserved.
...
### 3.2 Target-session isolation and fixed state number
...
covariate standardization to its HMM fit. The primary state count is fixed
a priori at \(K=2\), the smallest nontrivial switching model. No target-specific,
mouse-specific, or cohort-level state-number selection is performed.

For hazard tuning session \(u\ne s\), its predictive posterior is generated by a
separate \(K=2\) fit that excludes both \(s\) and \(u\). Thus target behavior
cannot enter hazard hyperparameter tuning, and tuning-session behavior cannot
enter its own HMM parameters. At least three active sessions per mouse are
required: a three-session mouse uses two sessions for each target fit and the
single remaining session for each nested tuning fit.

For behavior-only adequacy, \(K=1\) and \(K=3\) are fit under the same
target-held-out rule. Their likelihoods are sensitivities only; they never
change primary \(K\), hazard features, estimability, or v4.1 eligibility.
...
Each registered K=1, K=2, and K=3 fit uses the 20 integer JAX seeds
`4100, ..., 4119`,
...
training data marginal likelihood is retained. If no finite fit converges, the
fit key is checkpointed with typed reason
`hmm_no_converged_initialization`; no alternative \(K\), seed count, or
optimizer is substituted.

The emission/transition M-step is called only through the package's precompiled
JIT entry point. For a mouse with \(S\) active sessions, every HMM compilation
uses shape `(S-1, T_max, D)`, where `T_max` is that mouse's maximum raw-trial
count and `D` is the frozen design dimension. Exclude-two fits add one wholly
structural-masked dummy session. Dummy and trailing-padding rows contribute
exactly zero initial, transition, and emission sufficient statistics. The
exact-length per-session recompilation path is prohibited.

Every fit key is identified by mouse, \(K\), and sorted excluded sessions; its
manifest strictly binds the corresponding training sessions, fixed shape,
seeds, optimizer configuration, code commit, cache, preregistration, and
environment. At most five fit keys execute in one process. A completed fit is
atomically checkpointed with parameters, scaler, all-start diagnostics,
held-out scores, required predictive posteriors, and file checksums. A
statistically nonestimable fit key is atomically checkpointed with its typed
reason and all-start diagnostics. Native backend/resource failure does not
produce a valid statistical-failure checkpoint. The neural hazard workflow
consumes only a complete public checkpoint release and never refits an HMM.
...
Primary hazard models use the first component of the complete two-state \(q_t\);
the second component is the reference. No state is selected as task responsive,
no probability is thresholded, and no state is named “engaged.”
...
`M2` adds every product between the single predictive-state component and the
50-by-\(B\) neural-basis features to `M1`. It is a preregistered secondary model
evaluated with the same likelihood and aggregation rules.
...
nested fixed-\(K=2\) HMM: that HMM is fit only to same-mouse
...
The K=1 no-state hazard sensitivity uses the same seed, session, trial-count
weighting, and mouse-equal aggregation as the primary. K1/K2/K3 behavioral
adequacy is first averaged equally across target sessions within mouse, then
equally across mice; its t interval is diagnostic and has no coverage gate.

`M2-M1`, event/dF/F comparisons, fixed K1/K2/K3 behavior-adequacy summaries,
component coefficients, hazard curves, and v3.3 comparisons are secondary or
descriptive. Multiplicity is not used to select a primary conclusion.
...
- `not_applicable_k1` for the K=1 sensitivity only.
...
Native backend/resource failures block publication and are never recoded as
statistical nonestimability or source-integrity failures. A fully executed
registered diagnostic may report a typed nonestimable result while still
...
- target-held-out behavior likelihoods for K=1, K=2, and K=3, including
  per-session `K2-K1` and `K3-K2`;
- an events-only K=1 no-state `M1-M0` hazard sensitivity with independent
  external tuning and the primary aggregation rules; it has no M2 or dF/F run;
...
| Primary `K=2`; sensitivity `K=1,3` | `K=2` is the smallest nontrivial switching model without data-dependent state-number selection | `K=1` tests no switching and `K=3` tests added complexity; `K=4` was computationally disproportionate with few sessions | Target-held-out K1/K2/K3 behavior likelihoods; K1 no-state hazard | Sensitivity failure is reported but never changes primary K |
| 20 HMM starts | Controls local optima under fixed-shape checkpointed fits | Fewer starts raise local-optimum risk; more starts add compute after the registered reproducibility check | Report converged count and likelihood range for every fit key | Zero convergence is nonestimable |
...
- immutable r2 HMM checkpoint release, all K1/K2/K3 fit/start diagnostics,
  fitted parameters, behavior sensitivities, and predictive—not
  smoothed—posterior checks;
